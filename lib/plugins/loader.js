...
const SymbolGroupConfig = Symbol("GroupConfig")
...
class PluginsLoader {
    ...
    async load(isRefresh = false) {
        ...
        for (const file of files) {
            if (this.corePluginDirs.has(file.name.split("/")[0])) {
                coreFiles.push(file)
            } else {
        ...
        await Promise.allSettled(otherFiles.map(file => this.importPlugin(file, packageErr)))
    ...
    async importPlugin(file, packageErr) {
        const pluginBackup = global.plugin
        if (this.corePluginDirs.has(file.name.split("/")[0])) {
            global.plugin = BasePlugin
        }
        try {
            ...
            if (await installMissingDependency(error, file.name)) {
                return
            }
            ...
            global.plugin = pluginBackup
        }
    }

    async loadPlugin(file, p) {
        if (!p?.prototype || !(p.prototype instanceof BasePlugin)) {
            return
        }
        this.pluginCount++
        /** 初始化、定时任务实例 */
        ...
        if (plugin.init && (await plugin.init()) === "return") {
            return
        }
        /** 设置定时任务 */
    ...
    packageTips(packageErr) {
        ...
        Bot.makeLog("error", `安装插件后请 ${logger.red("bun install")} 安装依赖`, "Plugin")
        Bot.makeLog("error", `仍报错${logger.red("进入插件目录")} bun add 依赖`, "Plugin")
    ...
    async deal(e) {
        ...
        /** 检查黑白名单 */
        if (!this.checkBlack(e)) {
            return
        }
        /** 冷却 */
        if (!this.checkLimit(e)) {
            return
        }
        /** 处理事件 */
        this.dealEvent(e)
        ...
        const priority = []
        const eventPluginCache = new Map()
        const initRuntime = async () => {
            if (!e.runtime) {
                await Runtime.init(e)
            }
        }
        const getEventPlugin = async item => {
            if (!eventPluginCache.has(item)) {
                await initRuntime()
                eventPluginCache.set(
                    item,
                    Object.assign(new item.class(e), {
                        e
                    })
                )
            }
            return eventPluginCache.get(item)
        }
        for (const i of this.priority) {
            /** 判断是否启用功能，过滤事件 */
            if (
                this.checkDisable(
                    Object.assign(i.plugin, {
                        e
                    })
                ) &&
        ...
        for (const i of priority) {
            ...
            if (!lodash.isEmpty(context)) {
                ...
                for (const fnc in context) {
                    const plugin = await getEventPlugin(i)
    ...
    getGroupConfig(e) {
        if (!e[SymbolGroupConfig]) {
            Object.defineProperty(e, SymbolGroupConfig, {
                value: cfg.getGroup(e.self_id, e.group_id)
            })
        }
        return e[SymbolGroupConfig]
    }

    /** 判断权限 */
    filtPermission(e, v) {
        if (!v.permission || e.isMaster) {
            return true
        }
    ...
    dealEvent(e) {
        ...
        e.logText = `${logger.cyan(e.logText)}${logger.red(
            `[${lodash.truncate(e.msg || e.raw_message || [e.post_type, e.notice_type, e.sub_type].filter(i => i).join("."), {
                length: 100
            })}]`
        )}`
        ...
        /** 只关注主动at msg处理 */
        if (e.msg && e.isGroup) {
            const alias = this.getGroupConfig(e).botAlias
            for (const i of Array.isArray(alias)
                ? alias
                : [
                      alias
                  ]) {
    ...
    async count(e, type, msg) {
        ...
        if (cfg.bot.msg_type_count) {
            for (const i of Array.isArray(msg)
                ...
                types.push(`${type}:${i?.type || "text"}`)
            }
        }
        await this.saveCount(e, types)
    }
    ...
    async saveCount(e, type) {
        ...
        for (const i of [
            ...
            if (e.self_id) {
                scopes.push(`bot:${e.self_id}:${i}`)
            }
            if (e.user_id) {
                scopes.push(`user:${e.user_id}:${i}`)
            }
            if (e.group_id) {
                scopes.push(`group:${e.group_id}:${i}`)
            }
            for (const countType of types) {
                for (const scope of scopes) {
                    keys.push(`Yz:count:${countType}:${scope}`)
                }
            }
        }
    ...
    collectTask(task, name) {
        ...
        for (const i of Array.isArray(task)
            ? task
            : [
                  task
              ]) {
    ...
    checkLimit(e) {
        /** 禁言中 */
        if (e.group && (e.group.mute_left > 0 || (e.group.all_muted && !e.group.is_admin && !e.group.is_owner))) {
            return false
        }
        if (!e.message || e.isPrivate) {
            return true
        }

        const config = this.getGroupConfig(e)
    ...
    setLimit(e) {
        ...
        let config = this.getGroupConfig(e)
    ...
    onlyReplyAt(e) {
        ...
        let groupCfg = this.getGroupConfig(e)

        /** 模式0，未开启前缀 */
        if (groupCfg.onlyReplyAt === 0 || !groupCfg.botAlias) {
            return true
        }

        /** 模式2，非主人开启 */
        if (groupCfg.onlyReplyAt === 2 && e.isMaster) {
            return true
        }
    ...
    /** 判断是否启用功能 */
    checkDisable(p) {
        const groupCfg = this.getGroupConfig(p.e)
    ...
    watch(dirName, appName) {
        ...
        if (this.watcher[`${dirName}.${appName}`]) {
            return
        }
